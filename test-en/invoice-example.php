...
// invoice bulk save
// the save_bulk endpoint accepts two data schemas: one similar to the hardcoded format and a simplified one; both are shown in this example
    ...
    "data" => array(
        "data" => array(
            0 => array(
                ...
                "receipt_type_cr_id" => 188, // invoice block ID (if not specified, appended to the default block)
                "paymode_id" => 1, // payment method ID
                ...
                "partner_id" => 88, // invoice recipient company ID
                "partner_user_id" => 81, // invoice recipient company contact ID
                "partner_billing_id" => 135, // invoice recipient company billing address ID
                "partner_shipping_id" => 135, // invoice recipient company shipping address ID
                "currency_id" => 1, // invoice currency ID
            ...
            // invoice data content with simplified data
            1 => array(
                "receipt_type_id" => 8, // receipt type (8 - invoice, 13 - advance invoice, 33 - proforma invoice)
                ...
                "item"  =>  array(
                    0 => array(
                        "product" => array(
                            ...
                            "price" =>  0.00,  // product or service list price (added to the price list as the base sales price)
                        ),
                        "store_id" => 1, // store ID
                        "quantity" => 1, // quantity
                        "tax_id" => 1, // item tax rate
...
// hardcoded endpoint
$invoice_save = $invoice_client->save(array(
    "data" => array(
        "receipt_type_id" => 8, // receipt type (8 - invoice, 13 - advance invoice, 33 - proforma invoice)
        "receipt_type_cr_id" => 188, // invoice block ID (if not specified, appended to the default block)
        "paymode_id" => 1, // payment method ID
...
print_r($invoice_save);

// hardcoded close
$invoice_close = $invoice_client->close(array(
    "data" => array(
        "receipt_id" => 0, // receipt ID (following the example above, $invoice_save->data->id)
    )
));

print_r($invoice_close);
